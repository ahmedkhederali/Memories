...
function ARCSlider({ setImageID }) {
  ...
  return (
    <div style={{ width: 'auto', height: 500 }}>
      <div id=" text-start" className=" mt-5  m-auto" style={{direction:'rtl'}}>
            ...
            dataslider?.map((item, index) => (
              <LazyLoad key={item.id} height={200} offset={100}>
                <div
                  ...
                  key={item.id}
                  style={{ width: '15rem', height: '16rem', margin: '10px' }}
                >
                  ...
                  <ul className="list-group list-group-flush px-2 py-2" style={{direction:'rtl'}}>
                    <li className="list-group-item text-end" style={{ color: '#F29222' }}>
                      المستخدم: <span style={{ color: '#bdb9b9' }}>{item.author}</span>
                    </li>
                    <li className="list-group-item text-end" style={{ color: '#F29222' }}>
                      العنوان: <span style={{ color: '#bdb9b9' }}>{convertDateFormat(item.created_at)}</span>{' '}
                    </li>
                    <li className="list-group-item text-end" style={{ color: '#F29222' }}>
                    الإعجابات: <span style={{ color: '#bdb9b9' }}>{item.num_likes}</span>
                    </li>
                  </ul>
                </div>
              </LazyLoad>
